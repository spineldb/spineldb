...
mod tests {
    ...
    #[test]
    fn test_rpop_parses_key() {
        let c = RPop::parse(&[bs("k")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"k"));
    }

    #[test]
    fn test_rpop_with_no_args_is_error() {
        let r = RPop::parse(&[]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_rpop_with_too_many_args_is_error() {
        let r = RPop::parse(&[bs("k"), bs("extra")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_rpop_with_non_bulk_is_wrong_type() {
        let r = RPop::parse(&[RespFrame::Integer(1)]);
        assert!(matches!(r, Err(SpinelDBError::WrongType)));
    }

    #[test]
    fn test_rpop_to_resp_args_round_trips() {
        let c = RPop::parse(&[bs("k")]).unwrap();
