...
impl ParseCommand for Scard {
    fn parse(args: &[RespFrame]) -> Result<Self, SpinelDBError> {
        validate_arg_count(args, 1, "SCARD")?;
        Ok(Scard {
...
mod tests {
    ...
    #[test]
    fn test_scard_parses_key() {
        let c = Scard::parse(&[bs("myset")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"myset"));
    }

    #[test]
    fn test_scard_with_no_args_is_error() {
        let r = Scard::parse(&[]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_scard_with_too_many_args_is_error() {
        let r = Scard::parse(&[bs("k"), bs("extra")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_scard_with_non_bulk_is_wrong_type() {
        let r = Scard::parse(&[RespFrame::Integer(1)]);
        assert!(matches!(r, Err(SpinelDBError::WrongType)));
    }

    #[test]
    fn test_scard_to_resp_args_round_trips() {
        let c = Scard::parse(&[bs("k")]).unwrap();
