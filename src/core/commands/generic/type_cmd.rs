...
mod tests {
    ...
    #[test]
    fn test_type_parses_key() {
        let t = TypeInfo::parse(&[bs("k")]).unwrap();
        assert_eq!(t.key, Bytes::from_static(b"k"));
    }

    #[test]
    fn test_type_with_no_args_is_error() {
        let r = TypeInfo::parse(&[]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_type_with_too_many_args_is_error() {
        let r = TypeInfo::parse(&[bs("k"), bs("extra")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_type_with_non_bulk_is_wrong_type() {
        let r = TypeInfo::parse(&[RespFrame::Integer(1)]);
        assert!(matches!(r, Err(SpinelDBError::WrongType)));
    }

    #[test]
    fn test_type_to_resp_args_round_trips() {
        let t = TypeInfo::parse(&[bs("foo")]).unwrap();
        assert_eq!(t.to_resp_args(), vec![Bytes::from_static(b"foo")]);
    }
}
