...
// Konstanta Bumi dan karakter Geohash
...
fn encode_int_to_hash(mut bits: u64, len: usize) -> Result<String, SpinelDBError> {
    ...
    for i in (0..len).rev() {
        let idx = (bits & 0x1f) as usize; // Ambil 5 bit terbawah
...
/// Mengubah koordinat (longitude, latitude) menjadi skor 52-bit untuk ZSET.
pub fn coordinates_to_score(longitude: f64, latitude: f64) -> Result<f64, SpinelDBError> {
    ...
    // Presisi 11 menghasilkan 55 bit, kita akan pakai 52 bit teratas
...
/// Mengubah skor 52-bit dari ZSET kembali menjadi (longitude, latitude).
...
/// [BARU] Menentukan level presisi geohash berdasarkan radius.
...
/// [BARU] Mengubah prefix geohash menjadi rentang skor 52-bit.
