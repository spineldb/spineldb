...
mod tests {
    ...
    #[test]
    fn test_zlexcount_parses_args() {
        let c = ZLexCount::parse(&[bs("z"), bs("[a"), bs("[b")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"z"));
    }

    #[test]
    fn test_zlexcount_parses_exclusive_bounds() {
        let c = ZLexCount::parse(&[bs("z"), bs("(a"), bs("(b")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"z"));
    }

    #[test]
    fn test_zlexcount_parses_infinity() {
        let c = ZLexCount::parse(&[bs("z"), bs("-"), bs("+")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"z"));
    }

    #[test]
    fn test_zlexcount_with_too_few_args_is_error() {
        let r = ZLexCount::parse(&[bs("z"), bs("[a")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_zlexcount_with_too_many_args_is_error() {
        let r = ZLexCount::parse(&[bs("z"), bs("[a"), bs("[b"), bs("extra")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_zlexcount_with_non_bulk_key_is_wrong_type() {
        let r = ZLexCount::parse(&[RespFrame::Integer(1), bs("[a"), bs("[b")]);
        assert!(matches!(r, Err(SpinelDBError::WrongType)));
    }

    #[test]
    fn test_zlexcount_to_resp_args_round_trips() {
        let c = ZLexCount::parse(&[bs("z"), bs("[a"), bs("[b")]).unwrap();
