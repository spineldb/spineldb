// src/core/commands/zset/zrevrank.rs
...
impl ParseCommand for ZRevRank {
    fn parse(args: &[RespFrame]) -> Result<Self, SpinelDBError> {
        validate_arg_count(args, 2, "ZREVRANK")?;
        Ok(ZRevRank {
    ...
    async fn execute<'a>(
        ...
        if let Some(entry) = shard_cache_guard.get_mut(&self.key) {
            ...
            if let DataValue::SortedSet(zset) = &entry.data {
                if let Some(rank) = zset.get_rev_rank(&self.member) {
...
mod tests {
    ...
    #[test]
    fn test_zrevrank_parses_key_and_member() {
        let c = ZRevRank::parse(&[bs("z"), bs("m")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"z"));
        assert_eq!(c.member, Bytes::from_static(b"m"));
    }

    #[test]
    fn test_zrevrank_with_too_few_args_is_error() {
        let r = ZRevRank::parse(&[bs("z")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_zrevrank_with_too_many_args_is_error() {
        let r = ZRevRank::parse(&[bs("z"), bs("m"), bs("extra")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_zrevrank_with_non_bulk_key_is_wrong_type() {
        let r = ZRevRank::parse(&[RespFrame::Integer(1), bs("m")]);
        assert!(matches!(r, Err(SpinelDBError::WrongType)));
    }

    #[test]
    fn test_zrevrank_to_resp_args_round_trips() {
        let c = ZRevRank::parse(&[bs("z"), bs("m")]).unwrap();
        assert_eq!(
            c.to_resp_args(),
