...
mod tests {
    ...
    #[test]
    fn test_zcard_parses_key() {
        let c = ZCard::parse(&[bs("z")]).unwrap();
        assert_eq!(c.key, Bytes::from_static(b"z"));
    }

    #[test]
    fn test_zcard_with_no_args_is_error() {
        let r = ZCard::parse(&[]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_zcard_with_too_many_args_is_error() {
        let r = ZCard::parse(&[bs("z"), bs("extra")]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_zcard_with_non_bulk_is_wrong_type() {
        let r = ZCard::parse(&[RespFrame::Integer(1)]);
        assert!(matches!(r, Err(SpinelDBError::WrongType)));
    }

    #[test]
    fn test_zcard_to_resp_args_round_trips() {
        let c = ZCard::parse(&[bs("z")]).unwrap();
        assert_eq!(c.to_resp_args(), vec![Bytes::from_static(b"z")]);
    }
}
