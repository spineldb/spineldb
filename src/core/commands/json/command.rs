...
use super::json_clear::JsonClear;
...
impl ParseCommand for Json {
    ...
    fn parse(args: &[RespFrame]) -> Result<Self, SpinelDBError> {
        ...
        let subcommand = match sub_str.as_str() {
            ...
            "clear" => JsonSubcommand::Clear(JsonClear::parse(command_args)?),
            "del" | "forget" => JsonSubcommand::Del(JsonDel::parse(command_args)?),
            "get" => JsonSubcommand::Get(JsonGet::parse(command_args)?),
            "merge" => JsonSubcommand::Merge(JsonMerge::parse(command_args)?),
...
impl ExecutableCommand for Json {
    ...
    async fn execute<'a>(
        ...
        match &self.subcommand {
            ...
            JsonSubcommand::Clear(cmd) => cmd.execute(ctx).await,
            JsonSubcommand::Del(cmd) => cmd.execute(ctx).await,
            JsonSubcommand::Get(cmd) => cmd.execute(ctx).await,
            JsonSubcommand::Merge(cmd) => cmd.execute(ctx).await,
...
impl CommandSpec for Json {
    ...
    fn arity(&self) -> i64 {
        ...
        match &self.subcommand {
            ...
            JsonSubcommand::Clear(cmd) => cmd.arity(),
            JsonSubcommand::Del(cmd) => cmd.arity(),
            JsonSubcommand::Get(cmd) => cmd.arity(),
            JsonSubcommand::Merge(cmd) => cmd.arity(),
    ...
    fn flags(&self) -> CommandFlags {
        ...
        match &self.subcommand {
            ...
            JsonSubcommand::Clear(cmd) => cmd.flags(),
            JsonSubcommand::Del(cmd) => cmd.flags(),
            JsonSubcommand::Get(cmd) => cmd.flags(),
            JsonSubcommand::Merge(cmd) => cmd.flags(),
    ...
    fn get_keys(&self) -> Vec<Bytes> {
        ...
        match &self.subcommand {
            ...
            JsonSubcommand::Clear(cmd) => cmd.get_keys(),
            JsonSubcommand::Del(cmd) => cmd.get_keys(),
            JsonSubcommand::Get(cmd) => cmd.get_keys(),
            JsonSubcommand::Merge(cmd) => cmd.get_keys(),
...
mod tests {
    ...
    #[test]
    fn test_json_no_args_is_error() {
        let r = Json::parse(&[]);
        assert!(matches!(r, Err(SpinelDBError::WrongArgumentCount(_))));
    }

    #[test]
    fn test_json_unknown_subcommand_is_error() {
        let r = Json::parse(&[bs("FOO")]);
